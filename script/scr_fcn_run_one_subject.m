function rs = scr_fcn_run_one_subject(guiParams)
...
elseif guiParams.status == 2
    list_file.status = 'negative';
else
    list_file.status = 'unknown';
end
try
...
%% RUN 4 step
hwait = waitbar(0,'Current: Coregister PET to MRI', 'Name', 'Progress of 4 steps...');
tail = ',1';
...
try
    ...
    close(hwait);
    warndlg([guiParams.subject_name, ' [FAIL] at Step 1: Coregistration']);
    return;
end
...
try
    ...
    close(hwait);
    warndlg([guiParams.subject_name, ' [FAIL] at Step 2: Segmentation']);
    return;
end
...
try
    ...
    close(hwait);
    warndlg([guiParams.subject_name, ' [FAIL] at Step 3: Matching']);
    return;
end
...
try
    ...
    close(hwait);
    warndlg([guiParams.subject_name, ' [FAIL] at Step 4: Transform ROI template.']);
    return;
end
close(hwait);
rs = 1; % success