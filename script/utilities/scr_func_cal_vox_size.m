function v = scr_func_cal_vox_size(hdr)
% function v = scr_func_cal_vox_size(hdr)
% calculate voxel size, given hdr
    d = [];
    % distance along to z
    point1 = cor2mni([1 1 1], hdr.mat);
    point2 = cor2mni([1 1 2], hdr.mat);
    d(1) = calDist(point1, point2);
    
    % distance along to x
    point1 = cor2mni([1 1 1], hdr.mat);
    point2 = cor2mni([2 1 1], hdr.mat);
    d(2) = calDist(point1, point2);
    
    % distance along to y
    point1 = cor2mni([1 1 1], hdr.mat);
    point2 = cor2mni([1 2 1], hdr.mat);
    d(3) = calDist(point1, point2);
    
    % calculate voxel size
    v = d(1) * d(2) * d(3);
end

function d = calDist(p1, p2)
    d = sqrt( (p1(1)-p2(1))^2 + (p1(2)-p2(2))^2 + (p1(3)-p2(3))^2 );
end

function mni = cor2mni(cor, T)
% function mni = cor2mni(cor, T)
% convert matrix coordinate to mni coordinate
%
% cor: an Nx3 matrix
% T: (optional) rotation matrix
% mni is the returned coordinate in mni space
%
% caution: if T is not given, the default T is
% T = ...
%     [-4     0     0    84;...
%      0     4     0  -116;...
%      0     0     4   -56;...
%      0     0     0     1];
%
% xu cui
% 2004-8-18
% last revised: 2005-04-30

if nargin == 1
    T = ...
        [-4     0     0    84;...
         0     4     0  -116;...
         0     0     4   -56;...
         0     0     0     1];
end

cor = round(cor);
mni = T*[cor(:,1) cor(:,2) cor(:,3) ones(size(cor,1),1)]';
mni = mni';
mni(:,4) = [];
end